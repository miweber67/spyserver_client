 ...
 *
 */
#ifndef SS_CLIENT_IF_H
#define SS_CLIENT_IF_H
...
// Debugging tool; prints elements of a vector as hex digits separated by spaces
template <typename T>
void print_vec( std::vector<T>& outdata) {
   auto oldflags = std::cerr.flags(std::ios::hex | std::ios::right);
   auto oldfill = std::cerr.fill('0');
   for( size_t i = 0; i < outdata.size(); ++i) {
      std::cerr << std::hex << std::setw(2) << (unsigned int)(outdata[i]) << std::dec << " ";
   }
...
public:
  ...
  ss_client_if (const std::string    _ip,
                      const int      _port,
